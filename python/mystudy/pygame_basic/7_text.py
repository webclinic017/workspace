...
character_y_pos = screen_height - character_height     # 화면 세로 크기 가장 아래에 해다하는 곳에 위치 (세로)
...
enemy_y_pos = (screen_height / 2) - (enemy_height / 2)     # 화면 세로 크기 가장 아래에 해다하는 곳에 위치 (세로)
...
while running:
    ...
    # 타이머 집어 넣기
    # 경과 시간 계산
    elasped_time = (pygame.time.get_ticks() - start_ticks) / 1000   # 경과 시간(ms)을 1000으로 나누어서 초(s) 단위로 표시
    
    timer= game_font.render(str(int(total_time - elasped_time)), True, (0, 0, 0)) # 출력할 글자, True, 글자 색상
